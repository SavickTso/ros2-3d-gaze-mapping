...
class GazeSearcher:
    ...
    def depthmapupdate(self):
        flag = False
        if rclpy.ok():
            # self.node.get_logger().info("failed to get depth image, try again")
    ...
    def newton_search_iter(self, gaze3d_inferred, new_depth, max_iter):
        ...
        return gaze3d_inferred, zed2d, flag  # return the found Gazepoint in 3d coord
    ...
    def pointdisplay(self):
        ...
        # cv2.imshow("ZED Left view with Gaze point", image_ocv)
...
def SearchIt(node, init_z=None):
    ...
    while rclpy.ok():
        try:
            ...
            searcher.zedrgbupdate()
            searcher.pointdisplay()
            Gaze3d_Publisher(gaze3d, gaze3dpub)
            zedcoor_list.append(deepcopy(zed2d))
            # display the view of ZED and press ESC to exit the program
        except KeyboardInterrupt:
            # compute error
            searcher.node.get_logger().info("computing error")
    searcher.close()
